...
import { FirstPersonControls, Stats, Text, Line, Plane } from '@react-three/drei'
...
const Texture = ({ texture, position, text, size }) => {
  return ( 
      ...
      >
        ...
        <Line
          points={[[(size.width/2)-.3, 0,-0.01],[(size.width/2)-.3, 50,-0.01]]}
          color={"gray"}
          lineWidth={0.4}
          />
        <Line
          ...
          lineWidth={0.4}
          />
        <Text color="black" anchorX="left" anchorY="top" position={[size.width/-2,size.height/-2,0]}>
...
const Art = () => {
  return (
    <>
      {artData.map((art, i) => {
        return <Image key={i} url={art.fileName} text={art.text} position={[i*5,.5,0]} />
      })}
    </>
  )
}
